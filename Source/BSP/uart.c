...
// b5置1使能串口，置0使能SPI
    ...
    NST_WR_UART_REG(LCR_DLAB_MASK|LCR_STOP_MASK|UART_8BIT_MODE, LCR_OFFSET);    //0x87，停止位1.5个，位数8，解锁
    NSTusecDelay(5);
    //Delay();
    ...
    NST_WR_UART_REG(UART_8BIT_MODE, LCR_OFFSET);    //0x3,停止位1个，位数8，上锁
    NSTusecDelay(5);
    //Delay();
    
    NST_WR_UART_REG(0x01, FCR_OFFSET);  //0x01
    NSTusecDelay(5);
